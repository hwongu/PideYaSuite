...
public class RoleDAO extends Conexion<Role> {
    ...
    private static final String SELECT_MENUS_BY_ROLE_SQL =
        "SELECT mi.idMenuItem, mi.nombre AS menuNombre, mi.parentId, mi.orden " +
    ...
    /**
     * Lista los MenuItem asociados al Role con el nombre dado.
     *
     * @param roleNombre nombre del rol a buscar
     * @return lista de MenuItem asignados a ese rol
     * @throws SQLException en error SQL
     */
    public List<MenuItem> listarMenusPorRole(String roleNombre) throws SQLException {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    MenuItem m = new MenuItem(
                        rs.getInt("idMenuItem"),
                        rs.getString("menuNombre"),
                        rs.getInt("parentId"),
                        rs.getInt("orden")
                    );
                    lista.add(m);
